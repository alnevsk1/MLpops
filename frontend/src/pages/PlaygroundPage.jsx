...
function PlaygroundPage() {
  ...
  return (
    <div className="h-full flex flex-col">
      <h1 className="text-xl sm:text-2xl font-bold mb-4">Песочница: {model.name}</h1>

      {!hasToken && (
        <div className="bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 p-3 sm:p-4 rounded flex items-start sm:items-center gap-3 mb-6 text-sm sm:text-base">
      ...
      <div className="flex flex-col lg:flex-row gap-4 sm:gap-6 flex-1">
        {/* Левая панель - Ввод */}
        <div className="flex-1 bg-white dark:bg-gray-800 p-3 sm:p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col">
          <h3 className="font-semibold mb-2 text-sm sm:text-base">Ваш Промпт</h3>
          <textarea
            value={prompt}
            onChange={(e) => setPrompt(e.target.value)}
            disabled={!hasToken || loading}
            placeholder={model.output_type === 'TEXT' ? "Что такое пудж" : "Очень диталлизированный pudge из игры dota2"}
            className={`flex-1 w-full p-2 sm:p-3 border rounded text-sm sm:text-base dark:bg-gray-700 dark:border-gray-600 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 ${!hasToken ? 'opacity-50 cursor-not-allowed' : ''}`}
          />
          <button 
            onClick={handleRun}
            disabled={!hasToken || loading || !prompt}
            className={`mt-3 sm:mt-4 py-2 sm:py-3 px-3 sm:px-4 rounded flex items-center justify-center text-white text-sm sm:text-base font-medium transition-colors touch-none
              ${(!hasToken || loading || !prompt) ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 active:bg-blue-800'}
            `}
          >
        ...
        {/* Правая панель - Вывод */}
        <div className="flex-1 bg-gray-100 dark:bg-gray-900 p-3 sm:p-4 rounded-lg border border-gray-200 dark:border-gray-700 flex flex-col">
          <h3 className="font-semibold mb-2 text-sm sm:text-base">Результат</h3>
          
          <div className="flex-1 overflow-auto bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 p-3 sm:p-4">
            ...
            {result && model.output_type === 'IMAGE' && result.image_url && (
              <div className="flex flex-col items-center">
                ...
                <button
                  ...
                  className="mt-3 sm:mt-4 px-3 sm:px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded font-medium transition-colors text-sm"
                >
                  Скачать
                </button>
              </div>
